...
function getInterface() {
  ...
  d += "function rd(brief){var t1=pp.filter(function(o){return o.opi_score>=75;}).length,pur=pp.filter(function(o){return o.stage==='pursuing'||o.stage==='proposal';}).length,bt=brief||'The organism is working across '+pp.length+' active opportunities. '+t1+' are Tier 1 (OPI 75+). '+pur+' in pursuit or proposal stage.',mv=document.getElementById('mv');if(!mv)return;mv.innerHTML='<div class=\"bc\"><div class=\"bl\">Morning Briefing</div><div class=\"bt\">'+(function(s){var t=(s||'').trim();var di=t.indexOf('DASHBOARD:');if(di===0)t=t.slice(t.indexOf(':')+1).trim();var NL=String.fromCharCode(10);var BUL=String.fromCharCode(8226);var lines=t.split(NL);var out=[];for(var i=0;i<lines.length;i++){var l=lines[i];if(l.indexOf('### ')===0)l='<b>'+l.slice(4)+'</b>';else if(l.indexOf('## ')===0)l='<b>'+l.slice(3)+'</b>';else if(l.indexOf('# ')===0)l='<b>'+l.slice(2)+'</b>';else if(l.indexOf('- ')===0)l=BUL+' '+l.slice(2);out.push(l);}return out.join('<br>').slice(0,2000);})(bt)+'</div><div class=\"bm\">47 agents active</div></div>'+'<div class=\"ps\"><div class=\"pb\"><div class=\"pbl\">Active Opps</div><div class=\"pbv\">'+pp.length+'</div></div><div class=\"pb\"><div class=\"pbl\">Tier 1 OPI 75+</div><div class=\"pbv\">'+t1+'</div></div><div class=\"pb\"><div class=\"pbl\">In Pursuit</div><div class=\"pbv\">'+pur+'</div></div></div>'+'<div class=\"two\"><div><div class=\"sh\"><div class=\"st\">Active Pipeline</div></div>'+pp.map(opc).join('')+'</div><div><div class=\"sh\"><div class=\"st\">Organism Activity</div></div><div class=\"card\" style=\"max-height:480px;overflow-y:auto\">'+mm.slice(0,15).map(function(m){return'<div class=\"mi\"><div class=\"ma\">'+m.agent+'</div><div class=\"mt\">'+m.observation.slice(0,160)+'</div></div>';}).join('')+'</div></div></div>'+'<div style=\"margin-top:24px\"><div class=\"sh\"><div class=\"st\">Organism Decisions</div></div><div id=\"dc-list\" style=\"margin-top:12px\">'+(dc&&dc.length?dc.slice(0,20).map(function(x){return'<div class=\"card\" style=\"margin-bottom:10px;padding:14px 18px\"><div style=\"font-size:11px;font-weight:600;text-transform:uppercase;color:var(--gold);margin-bottom:4px\">'+x.agent+'</div><div style=\"font-size:13px;line-height:1.5;color:var(--text)\">'+x.observation.slice(0,220)+'</div><div style=\"font-size:11px;color:var(--muted);margin-top:6px\">'+new Date(x.created_at).toLocaleDateString()+'</div></div>';}).join(''):'<div style=\"padding:20px;color:var(--muted);font-size:13px\">No decisions yet. Run a session to generate organism decisions.</div>')+'</div>'+'<div id=\"det\" class=\"dp\"></div><div id=\"ra\" class=\"ra\"></div>'}";
