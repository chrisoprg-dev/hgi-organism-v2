...
:root{--navy:#1B2A4A;--gold:#C8A55A;--cream:#F8F6F1;--warm:#E8E4DC;--text:#2C2C2C;--muted:#6B6B6B;--green:#2D7A4F;--white:#FFF}
...
.oh{background:#E8F5EE;color:var(--green)}.om2{background:#FFF8E7;color:#B8860B}.ol{background:#FDEAEA;color:#C0392B}
.sbdg{font-size:10px;color:var(--muted);background:var(--warm);padding:2px 7px;border-radius:10px;text-transform:uppercase;letter-spacing:0.5px}
.db{font-size:10px;color:var(--muted)}
.mi{padding:11px 0;border-bottom:1px solid var(--warm)}
...
.sbtn{background:var(--navy);color:#fff;border:none;padding:9px 18px;border-radius:8px;font-size:13px;font-weight:500;cursor:pointer}
.sbtn:hover{background:#253960}
...
<body>
<nav class="sb">
  <div class="lg"><h1>HGI Organism</h1><p>V2 &bull; 47 Agents Active</p></div>
  <div class="nav">
  ...
  <div class="sf"><div class="sd"></div>Organism Active &bull; 47 Agents</div>
</nav>
<div class="main">
  <div class="tb">
    <div class="tv" id="vt">Morning Briefing</div>
    <div class="tm"><span class="tt" id="lc"></span><button class="rb" onclick="rs()">Run Session Now</button></div>
  </div>
  <div class="ct" id="mv"><div class="ld">Loading organism intelligence...</div></div>
</div>
<div class="ib">
  <input class="inp" id="ci" placeholder="Ask the organism anything or give it a direction..." onkeydown="if(event.key==='Enter')sc()">
  <button class="sbtn" onclick="sc()">Send</button>
</div>
<script>
...
function opc(o) {
  var pi = o.opi_score || 0;
  var pc = pi >= 80 ? 'oh' : pi >= 65 ? 'om2' : 'ol';
  var dd = o.due_date ? new Date(o.due_date).toLocaleDateString('en-US',{month:'short',day:'numeric'}) : '';
  var title = (o.title || '').length > 65 ? (o.title || '').slice(0,65) + '...' : (o.title || '');
  var stage = o.stage || 'identified';
  var id = JSON.stringify(o.id);
  return '<div class="oc" onclick="selO(' + id + ')"><div class="ot">' + title + '</div><div class="om"><span class="ob ' + pc + '">OPI ' + pi + '</span><span class="sbdg">' + stage + '</span>' + (dd ? '<span class="db">Due ' + dd + '</span>' : '') + '</div></div>';
}

function selO(id) {
  var o = pp.find(function(p) { return p.id === id; });
  if (!o) return;
  so = o;
  var dp = document.getElementById('det');
  if (!dp) return;
  dp.className = 'dp vis';
  var left = o.scope_analysis ? '<div class="dsl">Scope Analysis</div><div class="dtx">' + o.scope_analysis.slice(0,400) + '...</div>' : '';
  var right = o.research_brief ? '<div class="dsl">Research Brief</div><div class="dtx">' + o.research_brief.slice(0,400) + '...</div>' : '';
  var agentMem = mm.filter(function(m) { return m.opportunity_id === id; }).slice(0,4);
  if (agentMem.length) right += '<div class="dsl" style="margin-top:12px">Agent Intel</div>' + agentMem.map(function(m) { return '<div class="mi"><div class="ma">' + m.agent + '</div><div class="mt">' + m.observation.slice(0,150) + '...</div></div>'; }).join('');
  dp.innerHTML = '<div class="dtt">' + (o.title || '') + '</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:16px"><div>' + left + '</div><div>' + right + '</div></div>';
  dp.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
}

function rd(brief) {
  var tv = pp.reduce(function(s, o) { return s + (parseFloat((o.estimated_value || '').replace(/[^0-9.]/g,'')) || 0); }, 0);
  var wv = pp.reduce(function(s, o) { return s + (parseFloat((o.estimated_value || '').replace(/[^0-9.]/g,'')) || 0) * ((o.opi_score || 50) / 100); }, 0);
  var pur = pp.filter(function(o) { return o.stage === 'pursuing' || o.stage === 'proposal'; }).length;
  var t1 = pp.filter(function(o) { return o.opi_score >= 75; }).length;
  var bt = brief || 'The organism has been working overnight across ' + pp.length + ' active opportunities. ' + t1 + ' are Tier 1 (OPI 75+). ' + pur + ' in active pursuit or proposal stage. Run a session to generate a fresh morning briefing.';
  var mv = document.getElementById('mv');
  if (!mv) return;
  mv.innerHTML = '<div class="bc"><div class="bl">Morning Briefing</div><div class="bt">' + bt.slice(0,500) + '</div><div class="bm">dashboard_agent &bull; 47 agents active</div></div>' +
    '<div class="ps"><div class="pb"><div class="pbl">Active Opps</div><div class="pbv">' + pp.length + '</div><div class="pbs">in pipeline</div></div><div class="pb"><div class="pbl">Tier 1 OPI 75+</div><div class="pbv">' + t1 + '</div><div class="pbs">high priority</div></div><div class="pb"><div class="pbl">In Pursuit</div><div class="pbv">' + pur + '</div><div class="pbs">active proposals</div></div><div class="pb"><div class="pbl">Pipeline Value</div><div class="pbv">$' + (tv > 0.1 ? tv.toFixed(1) : '-') + 'M</div><div class="pbs">total estimated</div></div><div class="pb"><div class="pbl">Weighted Revenue</div><div class="pbv">$' + (wv > 0.1 ? wv.toFixed(1) : '-') + 'M</div><div class="pbs">probability-adj</div></div></div>' +
    '<div class="two"><div><div class="sh"><div class="st">Active Pipeline (' + pp.length + ')</div></div>' + pp.map(opc).join('') + '</div><div><div class="sh"><div class="st">Organism Activity</div></div><div class="card" style="max-height:480px;overflow-y:auto">' + mm.slice(0,15).map(function(m) { return '<div class="mi"><div class="ma">' + m.agent + '</div><div class="mt">' + m.observation.slice(0,160) + (m.observation.length > 160 ? '...' : '') + '</div><div class="md">' + new Date(m.created_at).toLocaleString('en-US',{month:'short',day:'numeric',hour:'numeric',minute:'2-digit'}) + '</div></div>'; }).join('') + '</div></div></div>' +
    '<div id="det" class="dp"></div><div id="ra" class="ra"></div><div style="height:60px"></div>';
}

function la() {
  Promise.all([
    fetch(B + '/api/pipeline').then(function(r) { return r.json(); }),
    fetch(B + '/api/memories').then(function(r) { return r.json(); }),
    fetch(B + '/api/briefing').then(function(r) { return r.json(); })
  ]).then(function(res) {
    pp = res[0] || [];
    mm = res[1] || [];
    rd((res[2] || {}).briefing);
    ...
    var mv = document.getElementById('mv');
    if (mv) mv.innerHTML = '<div class="ld">Error loading data: ' + e.message + '</div>';
  });
}
...
function sv(el, v) {
  ...
  var tmap = { dash:'Morning Briefing', pipe:'Pipeline Tracker', disc:'Opportunity Discovery', res:'Research & Analysis', win:'Winnability Scoring', prop:'Proposal Engine', fin:'Financial & Pricing', staff:'Recruiting & Bench', crm:'Relationship Intelligence', cont:'Content Engine', kb:'Knowledge Base', dig:'Weekly Digest', exec:'Executive Brief', scan:'Pipeline Scanner', scr:'Scraper Insights', chat:'System Chat' };
  var vt = document.getElementById('vt');
  if (vt) vt.textContent = tmap[v] || v;
  if (v === 'dash') { la(); return; }
  if (v === 'pipe') {
    var mv = document.getElementById('mv');
    if (mv) mv.innerHTML = '<div class="sh"><div class="st">All Opportunities (' + pp.length + ')</div></div>' + pp.map(opc).join('') + '<div id="det" class="dp" style="margin-top:20px"></div>';
    return;
  }
  var mv = document.getElementById('mv');
  if (mv) mv.innerHTML = '<div class="coming"><h2>Coming in Phase 2</h2><p>This module is being built in the next sprint.</p></div>';
}

function rs() {
  var btn = document.querySelector('.rb');
  if (btn) { btn.textContent = 'Running...'; btn.disabled = true; }
  fetch(B + '/run-session', { method: 'POST' }).then(function() {
    if (btn) { btn.textContent = 'Triggered!'; setTimeout(function() { btn.textContent = 'Run Session Now'; btn.disabled = false; }, 3000); }
...
function sc() {
  ...
  ra.textContent = 'Organism is thinking...';
  var ctx = so ? 'Current opportunity: ' + so.title + ' (OPI ' + so.opi_score + ', ' + so.stage + '). ' : '';
  var pc = pp.slice(0,5).map(function(o) { return o.title.slice(0,40) + ' OPI:' + o.opi_score; }).join(', ');
  fetch('https://api.anthropic.com/v1/messages', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ model: 'claude-sonnet-4-6', max_tokens: 800, system: 'You are the HGI Organism, 47 agents as one brain for HGI Global. Active pipeline: ' + pc + '. ' + ctx + 'Be direct and strategic. Every answer helps HGI win proposals.', messages: [{ role: 'user', content: msg }] })
  }).then(function(r) { return r.json(); }).then(function(d) {
    var t = (d.content || []).filter(function(b) { return b.type === 'text'; }).map(function(b) { return b.text; }).join('');
    if (ra) ra.textContent = t || 'No response.';
