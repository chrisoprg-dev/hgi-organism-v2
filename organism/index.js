...
if (url.startsWith('/api/produce-proposal') && req.method === 'POST') {
  ...
  // S150 RETIREMENT: HGI_PP_EXCLUSIONS fully retired 2026-04-27 per Christopher.
  // Exclusion list is now empty — no client triggers bid-refusal automatically.
  // This block remains as a hook for future targeted exclusions if Christopher
  // ever flags specific clients as no-bid (e.g. policy conflict, integrity hold).
  // Override (vestigial, kept for compatibility): POST body {forceExclusionOverride:true}.
  ...
  setImmediate(async () => {
    try {
      ...
      try {
        ...
        topPPText = '(PP selector unavailable — senior_writer must reference HGI_PP canon conservatively. No hard exclusions remain after S150 retirement; legitimate past performance with any cited client is acceptable.)';
      }
      ...
      var proposalPrompt = 'You are the HGI Global proposal production engine. Your job is to produce a COMPLETE, SUBMISSION-READY response document.\n\n' +
        ...
        '## HGI COMPANY PROFILE\n' + HGI + '\n\n' +
        '## TOP-RELEVANT PAST PERFORMANCE FOR THIS RFP (selected by HGI_PP selector; ranks 1-3 based on vertical match, agency-type match, scale, recency)\nFeature these 3 prominently in Past Performance and Experience sections. You may reference other HGI_PP canonical entries as supporting citations, but these 3 must be the primary feature. No current FEMA Public Assistance contract may be claimed.\n\n' + topPPText + '\n\n' +
        '## PAST PERFORMANCE REFERENCES (S150) — VERIFIED contacts for the 3 selected PPs above. Use ONLY these in the References / Tab 9 section. Render the contact name, title, organization, email, and phone exactly as listed. Do NOT invent, paraphrase, or fabricate contact details. If a PP has no reference in this section, write "[PROVIDED UPON REQUEST]" for that the contact info — do NOT make up a name or contact.\n\n' + (topPPReferencesText || 'No verified references on file for the selected PPs. For Tab 9 / References, write "[PROVIDED UPON REQUEST]" against each PP — never invent contact details.') + '\n\n' +
      ...
      try {
        ...
        try {
          ...
          var _canonChecks = [
            ...
            // S150 RETIREMENT: All EXCLUSION_* canon checks removed per Christopher 2026-04-27.
            // OPSB, PBGC, LIGA, TPCIGA all retired — HGI has legitimate engagements with these
            // entities and the exclusion rule was creating false positives. RTA + OPSB pending
            // formal addition to HGI_PP after Lou's full current contracts list arrives.
...
// S150 RETIREMENT: HGI_PP_EXCLUSIONS fully retired per Christopher's April 27 directive.
// OPSB and PBGC retired earlier in this session. LIGA and TPCIGA retired immediately
// after — Christopher confirmed HGI does have legitimate engagements with these
// entities (per OPI scoring prompt: "HGI has 20+ years TPCIGA/LIGA"), so the
// exclusion was a vestige of early-session hallucination concerns that have been
// superseded by the canonical HGI_PP constant + per-RFP PP selector.
// Empty list = no clients are bid-blocked. RTA + OPSB to be added to HGI_PP
// canonical list once Christopher's full current contracts list arrives from Lou.
var HGI_PP_EXCLUSIONS = [];
...
async function agentMethodologyResearcher(params, opts) {
  ...
  var briefSystem =
    ...
    '- No current FEMA Public Assistance contract may be claimed.\n' +
...
var L6_PAST_PERFORMANCE_CONFIG = {
  ...
  structural_requirements: [
    ...
    'Use only confirmed HGI past performance — refer to HGI_PP canonical entries for verifiable engagements',
  ...
  canon_violation_regex_set: [
    // S150 RETIREMENT: All pp_exclusion_* checks removed 2026-04-27 per Christopher.
    // OPSB, PBGC, LIGA, TPCIGA all retired. HGI has legitimate engagements with these
    // entities; the rule was creating false positives.
...
async function runSingleRefinementPass(opp, proposalText, reviewText) {
  ...
  var refineSystem =
    ...
    '\n7. Match the RFP structure exactly' +
    '\n8. NEVER list Tangipahoa as HGI past performance (geographic drift — Terrebonne is the canonical entity)' +
  ...
  for (var c = 0; c < Math.min(preFiltered.length, 50); c++) {
    try {
      ...
      var scorePrompt = HGI + '\n\nORGANISM INTELLIGENCE (use this to adjust scoring — relationships, competitor weaknesses, disasters, budget windows, and outcome lessons all affect how HGI should score this):' + huntContext +
        ...
        '\nPROGRAM ADMIN (federal): Score 70+ if federal/state program administration, PMO, case advisory, BPO.' +
...
async function generateStrategicThesis(opp, scope, financial, research, kbContext, topPPText, discriminatorsText, methodologyCorpusText, lessonsText) {
  ...
  var prompt = 'STRATEGIC THESIS DESIGN — produce the spine for a winning proposal.\n\n' +
    ...
    '"The firm New Orleans agencies trust to administer their hardest programs (Workers Comp TPA ' +
    'for City of New Orleans; Billing Appeals for SWBNO since 2011)" is strong ' +
