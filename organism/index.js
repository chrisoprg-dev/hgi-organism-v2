...
// === S133: PER-SECTION REFINEMENT LOOP ===
// Reads scoring_matrix from stored proposal_review, finds sections scoring below threshold,
// runs targeted regen + re-score per section via before/after diff (avoids fragile header
// parsing). Stops when section crosses threshold or no further improvement.
// Unlike L9 whole-proposal regen, this operates at section granularity — a single weak
// section no longer drags the overall PWIN plateau criterion.
// Separate from L9; exposed via /api/refine-weak-sections for test-before-integrate.
async function refineWeakSections(oppId, options) {
  var opts = options || {};
  var threshold = typeof opts.threshold === 'number' ? opts.threshold : 60;
  var maxIterPerSection = typeof opts.maxIterPerSection === 'number' ? opts.maxIterPerSection : 2;
  var dryRun = opts.dryRun !== false; // default true — don't rewrite proposal_content

  var report = {
    opp_id: oppId,
    threshold: threshold,
    max_iter_per_section: maxIterPerSection,
    dry_run: dryRun,
  ...
  try {
    var oppR = await supabase.from('opportunities')
      .select('id,title,agency,proposal_content,proposal_review,rfp_text,scope_analysis')
      .eq('id', oppId).single();
    var opp = oppR.data;
    if (!opp) { report.status = 'failed'; report.error = 'opportunity not found'; report.completed_at = new Date().toISOString(); return report; }
    if (!opp.proposal_content) { report.status = 'skipped'; report.error = 'no proposal_content'; report.completed_at = new Date().toISOString(); return report; }
    if (!opp.proposal_review) { report.status = 'skipped'; report.error = 'no proposal_review'; report.completed_at = new Date().toISOString(); return report; }

    // Parse scoring_matrix
    var reviewText = opp.proposal_review;
    var reviewJson = null;
    try {
      var jStart = reviewText.indexOf('{');
      var jEnd = reviewText.lastIndexOf('}');
      if (jStart >= 0 && jEnd > jStart) reviewJson = JSON.parse(reviewText.slice(jStart, jEnd + 1));
    } catch(e) { report.status = 'failed'; report.error = 'could not parse review JSON: ' + (e.message||e); report.completed_at = new Date().toISOString(); return report; }
    if (!reviewJson || !Array.isArray(reviewJson.scoring_matrix)) {
      report.status = 'failed';
      report.error = 'review has no scoring_matrix array';
      report.completed_at = new Date().toISOString();
      return report;
    }

    var matrix = reviewJson.scoring_matrix;
    report.sections_evaluated = matrix.length;
    report.baseline_pwin = reviewJson.pwin_estimate || null;

    function extractScore(s) {
      if (typeof s.evaluator_score === 'number') return s.evaluator_score;
      if (typeof s.estimated_score === 'number') return s.estimated_score;
      if (typeof s.pct === 'number') return s.pct;
      return null;
    }

    var weakSections = matrix.filter(function(s) {
      var sc = extractScore(s);
      return sc !== null && sc < threshold;
    });
    report.sections_below_threshold = weakSections.length;
    if (weakSections.length === 0) { report.status = 'no_weak_sections'; report.completed_at = new Date().toISOString(); return report; }

    var workingProposal = opp.proposal_content;
    var rfpCtx = String(opp.rfp_text || '').slice(0, 10000);

    for (var wi = 0; wi < weakSections.length; wi++) {
      var weak = weakSections[wi];
      var initScore = extractScore(weak);
      var sectionReport = {
        section: weak.section,
        initial_score: initScore,
        initial_rationale: String(weak.rationale || weak.note || '').slice(0, 400),
        iterations: [],
        final_score: initScore,
        improvement: 0
      };

      var currentScore = initScore;

      for (var iter = 0; iter < maxIterPerSection; iter++) {
        // Ask model for before/after diff — most reliable way to locate + revise a named section
        var regenPrompt = 'You are revising a single section of a government proposal. Return ONLY valid JSON with exactly two fields:\n' +
          '{"before_text": "the exact text currently in the proposal that constitutes this section (copy verbatim, minimum 300 characters, stay within this section only)", "after_text": "the fully revised section text"}\n\n' +
          'The before_text MUST appear verbatim in the proposal — the system will locate it and swap in after_text. If the section is too large to revise in one pass, select the weakest paragraph within the section as before_text and revise only that. Do not include section headers, section numbers, or content from other sections in either field. Do not add preamble or commentary.\n\n' +
          '## RFP CONTEXT (truncated)\n' + rfpCtx + '\n\n' +
          '## FULL PROPOSAL (current working version)\n' + workingProposal.slice(0, 40000) + '\n\n' +
          '## SECTION TO REVISE\n' + weak.section + '\n\n' +
          '## WEAKNESS TO ADDRESS\n' + String(weak.rationale || weak.note || 'section scored below threshold') + '\n\n' +
          '## JSON OUTPUT:';

        var regenResp, beforeText = '', afterText = '';
        try {
          regenResp = await claudeCall('Section regen: ' + weak.section, regenPrompt, 6000, { model: 'claude-sonnet-4-6', agent: 'refine_weak_sections_regen' });
          var rClean = String(regenResp||'').replace(/```json|```/g, '').trim();
          var rS = rClean.indexOf('{');
          var rE = rClean.lastIndexOf('}');
          if (rS >= 0 && rE > rS) {
            var rJ = JSON.parse(rClean.slice(rS, rE + 1));
            beforeText = String(rJ.before_text || '');
            afterText = String(rJ.after_text || '');
          }
        } catch(rErr) {
          sectionReport.iterations.push({ iter: iter + 1, error: 'regen failed: ' + (rErr.message||rErr) });
          break;
        }

        if (!beforeText || beforeText.length < 200) { sectionReport.iterations.push({ iter: iter + 1, error: 'before_text too short (' + beforeText.length + ' chars)' }); break; }
        if (!afterText || afterText.length < 100) { sectionReport.iterations.push({ iter: iter + 1, error: 'after_text too short' }); break; }
        if (workingProposal.indexOf(beforeText) < 0) { sectionReport.iterations.push({ iter: iter + 1, error: 'before_text not found verbatim in proposal' }); break; }

        // Re-score the new section text
        var rescorePrompt = 'Score this proposal section 0-100 as a government evaluator would. Return ONLY JSON: {"score": N, "rationale": "one paragraph"}.\n\n' +
          '## RFP CONTEXT\n' + rfpCtx + '\n\n' +
          '## SECTION NAME\n' + weak.section + '\n\n' +
          '## SECTION TEXT\n' + afterText + '\n\n' +
          '## JSON:';
        var newScore = null, newRat = '';
        try {
          var rsResp = await claudeCall('Section re-score: ' + weak.section, rescorePrompt, 1500, { model: 'claude-sonnet-4-6', agent: 'refine_weak_sections_rescore' });
          var rsClean = String(rsResp||'').replace(/```json|```/g, '').trim();
          var rsS = rsClean.indexOf('{');
          var rsE = rsClean.lastIndexOf('}');
          if (rsS >= 0 && rsE > rsS) {
            var rsJ = JSON.parse(rsClean.slice(rsS, rsE + 1));
            if (typeof rsJ.score === 'number') newScore = rsJ.score;
            newRat = String(rsJ.rationale || '');
          }
        } catch(rscErr) {
          sectionReport.iterations.push({ iter: iter + 1, before_length: beforeText.length, after_length: afterText.length, error: 'rescore failed: ' + (rscErr.message||rscErr) });
          break;
        }

        var accepted = newScore !== null && (currentScore === null || newScore > currentScore);
        sectionReport.iterations.push({
          iter: iter + 1,
          before_length: beforeText.length,
          after_length: afterText.length,
          revised_score: newScore,
          new_rationale: newRat.slice(0, 300),
          accepted: accepted
        });

        if (accepted) {
          workingProposal = workingProposal.replace(beforeText, afterText);
      ...
      sectionReport.final_score = currentScore;
      sectionReport.improvement = (initScore !== null && currentScore !== null) ? (currentScore - initScore) : null;
      report.sections_refined.push(sectionReport);
    }

    // Optionally persist
    if (!dryRun && workingProposal !== opp.proposal_content) {
      try {
        await supabase.from('opportunities').update({ proposal_content: workingProposal }).eq('id', oppId);
        report.proposal_content_updated = true;
      } catch(upErr) {
        report.proposal_content_updated = false;
        report.update_error = upErr.message || String(upErr);
      }
    } else {
      report.proposal_content_updated = false;
    }
    ...
    // Persist a summary memory entry so next session can see the run happened
    try {
      var rwsMemId = 's133_rws_' + oppId + '_' + Date.now();
      var rwsSummary = 'REFINE WEAK SECTIONS run: ' + report.sections_refined.length + ' sections processed; ' +
        report.sections_refined.filter(function(sr) { return sr.improvement > 0; }).length + ' improved; ' +
        'total improvement ' + report.sections_refined.reduce(function(a, sr) { return a + (sr.improvement || 0); }, 0) + ' points. ' +
        'Threshold=' + threshold + ', maxIter=' + maxIterPerSection + ', dryRun=' + dryRun + '.';
      await supabase.from('organism_memory').insert({
        id: rwsMemId,
        agent: 'refine_weak_sections',
        opportunity_id: oppId,
        entity_tags: 'section_refinement,s133,per_section_feedback_loop',
        observation: rwsSummary + '\n\n' + JSON.stringify(report).slice(0, 8000),
        memory_type: 'refinement_run',
        confidence: 'high',
        status: 'active'
      });
    } catch(memErr) { /* non-fatal */ }

    return report;
  } catch(outerErr) {
    report.status = 'error';
    report.error = outerErr.message || String(outerErr);
    report.stack = (outerErr.stack||'').slice(0, 500);
    report.completed_at = new Date().toISOString();
    return report;
  }
}

// === S133: /api/refine-weak-sections — POST {id, threshold?, maxIterPerSection?, dryRun?} ===
if (url.startsWith('/api/refine-weak-sections') && req.method === 'POST') {
  ...
  try {
    ...
    if (!rwsData.id) { res.end(JSON.stringify({ error: 'id required' })); return; }
    log('REFINE WEAK SECTIONS: oppId=' + rwsData.id + ', threshold=' + (rwsData.threshold||60) + ', dryRun=' + (rwsData.dryRun !== false));
    var rwsReport = await refineWeakSections(rwsData.id, {
      threshold: rwsData.threshold,
      maxIterPerSection: rwsData.maxIterPerSection,
      dryRun: rwsData.dryRun !== false
    });
    res.end(JSON.stringify(rwsReport, null, 2));
  } catch(rwsErr) {
    res.end(JSON.stringify({ error: rwsErr.message || String(rwsErr), stack: (rwsErr.stack||'').slice(0, 500) }));
  }
  return;
}
