# HGI ORGANISM V2 â COMPLETE INTERFACE BUILD SPECIFICATION
...
4. **Every data point is interactive.** Not just numbers â every label, status, field, score, name, date, percentage, badge. Hover or click anything and the organism has something to say, suggest, or explain about it.
...
## MODULE INVENTORY (18 total â all carry forward)
...
- TODAY'S PRIORITY briefing strip (organism's top recommendation â currently rendering raw JSON with backticks, BROKEN)
...
- THIS WEEK section (opportunities needing action this week â filter logic too narrow, misses April deadlines)
- ORGANISM DECISIONS full list (20 cards) â each card has:
  - Decision title
...
- WIN PATH: stage labels (Discover â Score â Research â Workflow â Proposal â Red Team â Export â Submit â WIN)
...
- Pipeline counters â real and accurate
- Scraper Status concept â right information, needs fixing
- Win Path concept â right idea, needs live opportunity markers
- Three-tier priority system (CRITICAL/HIGH/MEDIUM)

### What's Broken â Fix:
- TODAY'S PRIORITY renders raw JSON with backticks. Must parse and render as clean natural language prose. This is the first thing seen at login â cannot look broken.
...
- "Last think: 08:01 AM" â 5-hour gap visible when organism is supposed to always be thinking. Show gap time and make it meaningful.
- THIS WEEK filter too narrow â shows only 1 opportunity. Should show "next 30 days with pending action" not just current calendar week.
...
- Central Bidding DELAYED, Alabama/Georgia SETUP (never run) â needs more than a status label. Needs one-click fix action.
- Quick Start shortcuts are static/generic. Should be dynamic and context-aware based on what's actually pending.
- Win Path shows no live opportunity markers â just stage labels. No data on it.

### V2 Improvements â Add:
...
2. APPROVE button on every SYSTEM CAN EXECUTE card â one click triggers execution. MARK DONE + SNOOZE on YOUR ACTION cards. APPROVE BUILD + DEFER on BUILD REQUEST cards. The approval loop must close in the Dashboard.
...
5. Conversational input embedded in Dashboard â natural language field: "Ask the organism anything or give it a direction." Not a link to System Chat â right there on Dashboard.
6. Win Path becomes live visual with each active opportunity plotted as a marker at its current stage.
7. Organism agent health dots each identified and clickable â shows that agent's last output and current status.
...
- Dashboard is the organism's control center. Every module feeds decisions into it. Approvals on the Dashboard trigger execution in the corresponding module. The feedback loop: module produces data â dashboard surfaces decision â Christopher approves â organism executes â result returns to dashboard. This loop is missing in V1. V2 must complete it.
...
### What V1 Has (complete inventory):
- Subtitle: "RFP Decomposition → Executive Brief + OPI → Proposal Package"
- Opportunity selector at top: shows all 5 active pipeline opportunities with OPI scores and agency — click to load existing opp
- New opportunity entry form (below selector):
  ...
  - RELATIONSHIP INTEL / CONTEXT (large textarea — known relationships, budget intel, political context)
...
- Steps 2+ appear after Step 1 runs (not visible until triggered — need to run to document)

### Notes:
- This is the primary intake workflow for new opportunities
- Connects to the full orchestration pipeline: RFP → scope analysis → OPI → executive brief → proposal package
...
**Tab 1 — Hunt Engine:**
- Section header: "HUNT ENGINE — Scans all HGI verticals simultaneously"
...
- TOP OPPORTUNITIES section below — shows all results with IMMEDIATE/ACTIVE badge, TIER_1 badge, vertical tag, OPI score, organism analysis paragraph, → Workflow button, ? button, + Track button
...
**Tab 3 — Retrieve Docs:**
- Header: "DOCUMENT RETRIEVAL ENGINE — Paste any RFP URL"
...
**Tab 4 — Signals:**
- DISASTER SIGNALS section: "Recent declarations and events that will trigger FEMA PA/CDBG-DR RFP waves" — shows "Run Hunt to generate signals" (empty)
- FUNDING SIGNALS section: "HUD, FEMA, Treasury, USDA allocations that haven't produced RFPs yet — early warning" — shows "Run Hunt to generate signals" (empty)

**Tab 5 — Sources & Config:**
...
- STATE AGENCY WATCH LIST: full list for LA, TX, FL, MS, AL, GA — specific agencies named per state (GOHSEP, OCD-DRU, LHC, MEMA, Texas GLO, Florida DEO, FDEM, etc.)
